...
업로드 화면의 **수동 PDF 검토·자재 산출**은 실제 도면으로 1차 검토를 할 수 있는 브라우저 전용 작업공간입니다.

- PDF만 여러 개 추가할 수 있으며 파일당 50MB로 제한합니다.
- PDF 원본과 도면 유형, 수동 사양 검토 입력값은 서버로 전송하지 않고 이 브라우저의 IndexedDB에 저장합니다.
- 페이지 이동·확대/축소와 도면 유형(평면도·입면도·단면도·상세도·기타) 수동 지정을 지원합니다.
- 사양 항목은 기본 `검토 필요`이며, `확인 완료` 항목만 수동 산출표에 반영합니다. 단위가 다른 값은 합산하지 않습니다.
- 자동 판독 예시 데이터는 만들지 않습니다. 모든 산출값은 사용자가 입력·확인한 값입니다.

### 실제 PDF 12페이지 테스트 순서

1. `npm run dev`를 실행하고 업로드 화면의 **PDF 여러 개 추가**를 누릅니다.
2. 평면도·입면도·단면도·상세도가 포함된 실제 PDF 파일들을 선택합니다. 한 파일이 50MB를 넘으면 추가되지 않습니다.
3. 목록에서 파일을 하나씩 선택하고 페이지 이동 버튼으로 12페이지를 모두 확인합니다. 각 파일의 도면 유형을 수동 지정합니다.
4. 각 페이지에서 확인한 자재를 **항목 추가**로 입력하고, 항목명·규격·단위·수량·단가·재고·신뢰도·검토 메모를 채웁니다.
5. 원본 도면과 대조한 행만 `확인 완료`로 바꿉니다. 수량과 단위가 다른 항목은 별도 행으로 둡니다.
6. 수동 자재 산출표에서 확인 수량, 재고, 부족 수량, 신규 발주 수량, 금액을 확인합니다.
7. 새로고침 후 PDF 목록과 입력값이 복원되는지 확인합니다. 브라우저 저장소를 지우면 해당 작업공간 데이터도 삭제됩니다.

이번 1차 작업공간에는 자동 추출, 3D 생성, 발주 전송, CAD/BIM(DWG/DXF/IFC) 파서 연동을 넣지 않았습니다. 기존 자동 분석·재고·자투리·절단 최적화는 별도 흐름으로 유지됩니다.
...
src/
  components/
    ...
    ManualPdfReviewWorkspace.vue  # IndexedDB 기반 PDF 수동 검토·산출 작업공간
  modules/
